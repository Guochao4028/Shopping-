...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName{
    ...
    self.interval  = self.interval == 0? 18 : self.interval ;
    ...
    if ((starWidth != 0) && (starHeight != 0)) {
        for (int i = 0; i < self.numberOfStar; i ++) {
            ...
            CGFloat x = i * (starWidth + self.interval);
            imageView.frame = CGRectMake(x, 0, starWidth, starHeight);
            [view addSubview:imageView];
            self.viewWidth = x+starWidth;
        }
    }else{
        for (int i = 0; i < self.numberOfStar; i ++) {
            UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point{
    ...
    if (self.viewWidth > 0) {
        ...
        if (p.x > self.viewWidth) {
            p.x = self.viewWidth;
        }
        
        NSString * str = [NSString stringWithFormat:@"%0.2f",p.x / self.viewWidth];
        float score = [str floatValue];
        p.x = score * self.viewWidth;
...
- (void)setStarSize:(CGSize)starSize {
    ...
    self.starBackgroundView = nil;
    
    [self.starForegroundView removeFromSuperview];
    self.starForegroundView = nil;
    
    self.starBackgroundView = [self buidlStarViewWithImageName:@"StarImage"];
    self.starForegroundView = [self buidlStarViewWithImageName:kFOREGROUND_STAR];
    [self addSubview:self.starBackgroundView];
    [self addSubview:self.starForegroundView];
