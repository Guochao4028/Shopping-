                 ...
                 completion:(void(^)(NSError *error))aCompletion
{
    ...
    do {
        if (self.recorder && self.recorder.isRecording) {
            error = [NSError errorWithDomain:SLLocalizedString(@"正在进行录制") code:-1 userInfo:nil];
            break;
        }
        ...
        if (error) {
            error = [NSError errorWithDomain:SLLocalizedString(@"AVAudioSession SetCategory失败") code:-1 userInfo:nil];
            break;
        }
        ...
        if(error || !self.recorder) {
            self.recorder = nil;
            error = [NSError errorWithDomain:SLLocalizedString(@"文件格式转换失败") code:-1 userInfo:nil];
            break;
        }
        ...
        if (!ret) {
            [self _stopRecord];
            error = [NSError errorWithDomain:SLLocalizedString(@"准备录制工作失败") code:-1 userInfo:nil];
        }
