...
{
    if (error) {
        [self showAlertWithMessage:SLLocalizedString(@"自动登录失败，请重新登录")];
...
- (void)userAccountDidLoginFromOtherDevice
{
    ...
    [self showAlertWithMessage:SLLocalizedString(@"你的账号已在其他地方登录")];
...
{
    [[EMClient sharedClient] logout:NO];
    [self showAlertWithMessage:SLLocalizedString(@"你的账号已被从服务器端移除")];
...
{
    [[EMClient sharedClient] logout:NO];
    [self showAlertWithMessage:SLLocalizedString(@"账号被禁用")];
                                       ...
                                       ext:(NSString *)aExt
{
    ...
    [self showAlertWithTitle:SLLocalizedString(@"多设备[好友]") message:message];
}
                                     ...
                                     ext:(id)aExt
{
    ...
    [self showAlertWithTitle:SLLocalizedString(@"多设备[群组]") message:message];
}
               ...
               reason:(EMGroupLeaveReason)aReason
{
    ...
    if (aReason == EMGroupLeaveReasonBeRemoved) {
        str = [NSString stringWithFormat:SLLocalizedString(@"你被踢出群组: %@ [%@]"), aGroup.subject, aGroup.groupId];
    } else if (aReason == EMGroupLeaveReasonDestroyed) {
        str = [NSString stringWithFormat:SLLocalizedString(@"群组被解散: %@ [%@]"), aGroup.subject, aGroup.groupId];
    }
...
{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:[NSString stringWithFormat:SLLocalizedString(@"group.inviteSomeone"), aInviter, aGroup.subject, aGroup.groupId] delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
    [alertView show];
}
...
- (void)groupInvitationDidDecline:(EMGroup *)aGroup
                          invitee:(NSString *)aInvitee
                           reason:(NSString *)aReason
{
    NSString *message = [NSString stringWithFormat:SLLocalizedString(@"group.declinedInvite"), aInvitee, aGroup.subject];
...
- (void)groupInvitationDidAccept:(EMGroup *)aGroup
                         invitee:(NSString *)aInvitee
{
    NSString *message = [NSString stringWithFormat:SLLocalizedString(@"group.acceptedInvite"), aInvitee, aGroup.subject, aGroup.groupId];
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:message delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
    [alertView show];
}
...
{
    NSString *msg = [NSString stringWithFormat:SLLocalizedString(@"'%@'同意了你的好友请求"), aUsername];
    [self showAlertWithTitle:@"O(∩_∩)O" message:msg];
}

- (void)friendRequestDidDeclineByUser:(NSString *)aUsername
{
    NSString *msg = [NSString stringWithFormat:SLLocalizedString(@"'%@'拒绝了你的好友请求"), aUsername];
    [self showAlertWithTitle:@"O(∩_∩)O" message:msg];
}
...
- (void)handlePushChatController:(NSNotification *)aNotif
{
    ...
    if (model) { // 跳转到聊天界面
        ...
        [[EMDemoHelper currentDisplayViewController].navigationController pushViewController:controller animated:YES];
    }
}
...
+ (UIViewController *)currentDisplayViewController {
    ...
    resultVC = [EMDemoHelper loopGetCurrentVC:[[UIApplication sharedApplication].keyWindow rootViewController]];
    while (resultVC.presentedViewController) {
        resultVC = [EMDemoHelper loopGetCurrentVC:resultVC.presentedViewController];
    }
    return resultVC;
}
...
+ (UIViewController *)loopGetCurrentVC:(UIViewController *)vc {
    ...
    if ([vc isKindOfClass:[UINavigationController class]]) {
        return [EMDemoHelper loopGetCurrentVC:[(UINavigationController *)vc topViewController]];
    } else if ([vc isKindOfClass:[UITabBarController class]]) {
        return [EMDemoHelper loopGetCurrentVC:[(UITabBarController *)vc selectedViewController]];
    }else {
        return vc;
    }
    return nil;
