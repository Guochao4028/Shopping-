...
- (UILabel *)titleLabel{
    if(!_titleLabel){
        ...
        _titleLabel.text = NSLocalizedStringFromTableInBundle(@"Fast speed play", nil, resourceBundle, nil);//SLLocalizedString(@"倍速播放");
...
-(void)layoutSubviews{
    ...
    for (int i = 0; i<4; i++) {
        ...
        switch (i) {
            case 0:
                btn.speedLabel.text =  NSLocalizedStringFromTableInBundle(@"Nomal", nil, resourceBundle, nil);//SLLocalizedString(@"正常");
                break;
            case 1:
