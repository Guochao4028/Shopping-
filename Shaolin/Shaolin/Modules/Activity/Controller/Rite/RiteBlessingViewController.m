...
//    [headImageView sd_setImageWithURL:[NSURL URLWithString:@""] placeholderImage:[UIImage imageNamed:@"default_small"]];
    ...
    blessingWordsLabel.textColor = KTextGray_333;
...
- (WJMCheckBoxView *)createCheckBoxView:(NSArray *)datas{
    ...
    config.imageSize = 15;
    config.space = 5;
    config.normalFont = kRegular(15);
    config.normalImage = [UIImage imageNamed:@"radioNormal"];
    config.selectImage = [UIImage imageNamed:@"radioSelected_yellow"];
...
- (void)saveBlessingWordsView {
    if (self.riteBlessingModel.video.length){
        ...
        [self.saveButton setTitleColor:KTextGray_333 forState:UIControlStateNormal];
...
- (UIButton *)sharedButton{
    if (!_sharedButton){
        ...
        [_sharedButton setBackgroundImage:[UIImage imageNamed:@"shareYellow"] forState:UIControlStateNormal];
...
- (UIView *)blessingWordsViewScreenshot{
    if (!_blessingWordsViewScreenshot){
        ...
        _blessingWordsViewScreenshot.backgroundColor = [UIColor colorWithWhite:1 alpha:0.9];
    }
...
- (UIView *)neitanView{
    if (!_neitanView){
        ...
        _neitanView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.9];
    }
    return _neitanView;
}
...
- (UIView *)neitanViewFornt{
    if (!_neitanViewFornt){
        ...
        _neitanViewFornt.backgroundColor = [UIColor clearColor];
    }
...
- (UIView *)neitanViewBack {
    if (!_neitanViewBack){
        ...
        _neitanViewBack.backgroundColor = [UIColor clearColor];
    }
