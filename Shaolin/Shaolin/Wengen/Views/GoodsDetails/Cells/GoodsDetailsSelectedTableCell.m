...
-(void)setAddressModel:(AddressListModel *)addressModel{
    ...
    if (addressModel == nil) {
        ...
        NSString *str;
           if ([addressModel.country_s isEqualToString:SLLocalizedString(@"中国")] == NO) {
               str  = [NSString stringWithFormat:@"%@ %@ %@ %@ %@",addressModel.country_s, addressModel.province_s, addressModel.city_s, addressModel.re_s, addressModel.address];
           }else{
              str = [NSString stringWithFormat:@"%@ %@ %@ %@", addressModel.province_s, addressModel.city_s, addressModel.re_s, addressModel.address];
           }
...
-(void)setFeeStr:(NSString *)feeStr{
    ...
    if ([feeStr isEqualToString:@"0"] == YES ||feeFloat == 0) {
        ...
        [self.feeLabel setText:[NSString stringWithFormat:SLLocalizedString(@"%@元"),feeStr]];
    }
