...
+ (void)lgf_SaveImageToPhoto:(UIImage *)image saveSuccess:(void(^)(void))saveSuccess saveFailure:(void(^)(NSString *error))saveFailure {
    [LGFAllPermissions lgf_GetPhotoAutoPermission:^(BOOL isHave) {
        if (isHave) {
            ...
            [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                ...
                if (@available(iOS 9.0, *)) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        lgf_HaveBlock(saveFailure, SLLocalizedString(@"保存失败，系统版本过低"));
                    });
                }
            } completionHandler:^(BOOL success, NSError * _Nullable error) {
                if (success == NO) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        lgf_HaveBlock(saveFailure, NSLocalizedString(@"因为系统原因, 无法访问相册", nil));
                    });
                    return;
                }
                // 2.获得相簿
                ...
                [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                    ...
                    if (success == NO) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            lgf_HaveBlock(saveFailure, SLLocalizedString(@"保存失败"));
                        });
                    } else {
                        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                lgf_HaveBlock(saveFailure, SLLocalizedString(@"请进入手机设置，为我们打开保存相册的权限"));
            });
        }
    }];
}
...
+ (PHAssetCollection *)createdAssetCollection{
    ...
    for (PHAssetCollection *assetCollection in assetCollections) {
        if ([assetCollection.localizedTitle isEqualToString:SLLocalizedString(@"我的相册")]) {
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChangesAndWait:^{
        // 创建相簿的请求
        assetCollectionLocalIdentifier = [PHAssetCollectionChangeRequest creationRequestForAssetCollectionWithTitle:SLLocalizedString(@"我的相册")].placeholderForCreatedAssetCollection.localIdentifier;
    } error:&error];
...
+ (BOOL)lgf_IsGoodNetSpeed {
    ...
    if (speed < 100) {
        NSString *string = NSLocalizedString(@"您当前网速低于【100kb/s】,暂时不能开启直播；请先设置好您的网络要求", nil);
...
// data: @[@[@{@"key":SLLocalizedString(@"尺码"),@"name",@"L"},@{@"key":SLLocalizedString(@"尺码"),@"name",@"M"}], @[@{@"key":SLLocalizedString(@"颜色"),@"name",SLLocalizedString(@"黑色")},@{@"key":SLLocalizedString(@"颜色"),@"name",SLLocalizedString(@"红色")}]]
    ...
    [[LGFNetwork lgf_Once] lgf_POST:urlString parameters:@{} success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
        ...
        if (sourceArray.count >= 1) {
            ...
            if ([LGFAllMethod lgf_JudgeStoreVersion:storeVersion withAppVersion:appVersion]) {
                ...
                lgf_HaveBlock(failure, SLLocalizedString(@"当前为最新版本"));
            }
        }
