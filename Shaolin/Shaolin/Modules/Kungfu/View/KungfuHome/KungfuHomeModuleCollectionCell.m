...
- (void)layoutSubviews {
    ...
    if ([self.iconName isEqualToString:@"new_class_notiIcon"]) {
        if (IsNilOrNull(self.messageNum) || self.messageNum.length == 0 || [self.messageNum isEqualToString:@"0"] || [self.messageNum isEqualToString:@"(null)"]) {
            self.imageIcon.image = [UIImage imageNamed:@"new_class_notiIcon"];
    //        self.numLabel.hidden = YES;
        } else {
            self.imageIcon.image = [UIImage imageNamed:@"new_class_notiIcon_noRead"];
        }
    } else {
...
- (void)setupUI {
    ...
    [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.imageIcon.mas_bottom).offset(4);
