...
//        [self.fullScreenBtn setImage:[UIImage imageNamed:@"kr-video-player-fullscreen"] forState:UIControlStateNormal];
        ...
        self.currentTimeLabel = [[UILabel alloc]initWithFrame:CGRectMake(45, 15,30, 15)];
        self.currentTimeLabel.text = @"00:00";
        self.currentTimeLabel.textColor = [UIColor whiteColor];
        self.currentTimeLabel.textAlignment = NSTextAlignmentCenter;
        self.currentTimeLabel.font = kRegular(11);
        self.totalTimeLabel = [[UILabel alloc]init];
        self.totalTimeLabel.textAlignment = NSTextAlignmentCenter;
        self.totalTimeLabel.font = kRegular(11);
...
-(void)layoutSubviews
{
    ...
    CGFloat progressWidth =width-2*(self.startBtn.frame.size.width+self.currentTimeLabel.frame.size.width)+30;
    NSLog(@"%f   ",progressWidth);
    self.progressView.frame = CGRectMake(92.5,22.5,170,10);
    
    self.totalTimeLabel.frame = CGRectMake(width-75,15,30,15);
    self.videoSlider.frame = CGRectMake(90,17.5,190 ,10);
    NSLog(@"%f",self.videoSlider.frame.size.width);
