...
- (BOOL)handleOpenURL:(NSURL *)url {
    ...
    } else if ([host isEqualToString:@"safepay"]){//支付宝
...
- (void)payByThirdpartyType:(NSString *)thirdpartyType dict:(NSDictionary *)dict {
    if (![dict objectForKey:DATAS]) {
        NSString *errorStr = SLLocalizedString(@"支付失败");
...
- (void)aliPay:(NSDictionary *)dict{
    NSString *appScheme = @"shaolin";
    NSString *orderString = dict[DATAS];
    // NOTE: 调用支付宝 开始支付
