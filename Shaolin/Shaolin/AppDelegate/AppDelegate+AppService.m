...
#pragma mark - 内购异常订单检测
- (void)checkIAPOrder {
    // 获取所有本地存储的
    [WSIAPManager getAllWaitTransactionAndBlock:^(NSArray * _Nonnull unfinishedList) {
        
        if (unfinishedList && unfinishedList.count) {
            // 有异常订单，开始验证异常订单
            [ShaolinProgressHUD defaultSingleLoadingWithText:@"正在恢复异常订单"];
            for (int i = 0; i < unfinishedList.count; i++) {
                WSIAPModel *iapModel = unfinishedList[i];
                // 验证最后一个异常订单的时候才弹窗
                BOOL isLast = i==unfinishedList.count-1?YES:NO;
//                [WSIAPManager updateLocalIapModel:iapModel checkType:WSIAPCheckFinish];
                [self checkReceipWithIapModel:iapModel isLast:isLast];
            }
        }
    }];
}
...
/// @param isLast 异常订单可能有多个，最后一个异常订单结束后才弹窗
- (void) checkReceipWithIapModel:(WSIAPModel *)iapModel
    ...
    [[MeManager sharedInstance] postIAPCheckWithReceipt:iapModel.receiptString ordercode:orderCode callback:^(NSString * _Nonnull code, BOOL result) {
        ...
        if ((otherError || saveError)) {
            [self showAlertWithTitle:@"购买失败" msg:@"如果您已扣款成功，请稍后再次尝试重新启动App或联系客服人员，将会恢复您的订单"];
        } else {
            // 以下情况都是code==200
            WSIAPCheckType checkType;
            if (result) {
                // code == 200且result == true ，证明苹果验证通过且后台存储成功
                // 成功后，将本地存储的内购订单的状态变为已验证
                checkType = WSIAPCheckFinish;
                if (orderCode.length == 0) {
                    // orderCode为空，证明用户扣款成功，但是没有关联上开发者定义的订单号
                    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
                    pasteboard.string = iapModel.transactionId;
                    [self showAlertWithTitle:@"异常订单号已复制" msg:[NSString stringWithFormat:@"异常订单号为：%@，请联系客服处理，您可以继续本次支付",iapModel.transactionId]];
                } else {
                    if (isLast) {
                        [self showAlertWithTitle:@"" msg:@"异常订单已处理完成，您可以继续使用App"];
                    }
                }
            } else {
                /*
            ...
            [WSIAPManager updateLocalIapModel:iapModel checkType:checkType];
        }
    }];
}
...
- (void) showAlertWithTitle:(NSString *)title msg:(NSString *)msg {
    ...
    [[SLRouteManager findCurrentShowingViewController] presentViewController:alertController animated:YES completion:nil];
}
