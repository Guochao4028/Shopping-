...
+ (NSString *)lgf_TimeStampSwitchPublishTimeStr:(NSInteger)timeStamp {
    if (timeStamp < 0) {
        return SLLocalizedString(@"时间错误");
    }
    NSInteger currentDateStamp = [self lgf_GetNowTimeStamp];
    NSInteger interval = currentDateStamp - timeStamp;
    if (interval < 60) {
        return SLLocalizedString(@"刚刚");
    } else if (interval < 3600) {
        return [NSString stringWithFormat:SLLocalizedString(@"%ld分钟前"), (long)(interval / 60)];
    } else if (interval < 3600 * 24 && interval >= 3600) {
        return [NSString stringWithFormat:SLLocalizedString(@"%ld小时前"), (long)(interval / 3600)];
    }else if (interval < 3600 * 24 * 2 && interval >= 3600 * 24) {
        return SLLocalizedString(@"昨天");
    } else if (interval < 3600 * 24 * 7 && interval > 3600 * 24 * 2) {
        return [NSString stringWithFormat:SLLocalizedString(@"%ld天前"), (long)(interval / (3600 * 24))];
    }  else if (interval < 3600 * 24 * 30 && interval >= 3600 * 24 * 7) {
        return [NSString stringWithFormat:SLLocalizedString(@"%ld周前"), (long)(interval / (3600 * 24 * 7))];
    } else if (interval < 3600 * 24 * 365 && interval >= 3600 * 24 * 30) {
        return [NSString stringWithFormat:SLLocalizedString(@"%ld个月前"), (long)(interval / (3600 * 24 * 30))];
    } else if (interval >= 3600 * 24 * 365) {
        return [NSString stringWithFormat:SLLocalizedString(@"%ld年前"), (long)(interval / (3600 * 24 * 365))];
    }
    return @"";
}
...
+ (NSString *)lgf_GetNumStrWithNum:(NSUInteger)num unitType:(lgf_UnitType)unitType unitStrType:(lgf_UnitStrType)unitStrType {
    ...
    if (unitType == lgf_OnlyQian) {
        if (num < 1000) {
            ...
            str = [NSString stringWithFormat:@"%.1f%@",countUse ,unitStrType == lgf_SmallPinyin ? @"q" : unitStrType == lgf_BigPinyin ? @"Q" : SLLocalizedString(@"千")];
        }
        ...
        if (num < 10000) {
            ...
            str = [NSString stringWithFormat:@"%.1f%@",countUse ,unitStrType == lgf_SmallPinyin ? @"w" : unitStrType == lgf_BigPinyin ? @"W" : SLLocalizedString(@"万")];
        }
        ...
        if (num < 10000) {
            ...
            str = [NSString stringWithFormat:@"%.1f%@",countUse ,unitStrType == lgf_SmallPinyin ? @"y" : unitStrType == lgf_BigPinyin ? @"Y" : SLLocalizedString(@"亿")];
        }
...
+ (NSString *)lgf_GetVideoTimeLength:(NSUInteger)timeLength delimit:(NSString *)delimit TimeFormatType:(lgf_TimeFormatType)TimeFormatType {
    ...
    if (TimeFormatType == lgf_MS) {
        ...
        str = [NSString stringWithFormat:SLLocalizedString(@"%02ld分%02ld秒"), (long)(timeLength / 60) , (long)(timeLength % 60)];
    } else if (TimeFormatType == lgf_CCharacterHMS) {
        str = [NSString stringWithFormat:SLLocalizedString(@"%02ld点%02ld分%02ld秒"), (long)(timeLength / 3600), (long)((timeLength % 3600) / 60), (long)(timeLength % 60)];
    }
    return str;
}
