...
@interface DraftEditVideoViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@property(nonatomic,strong) NSString * firstVideoTime;
@property(nonatomic,strong) NSString * firstVideoUrl;
@property(nonatomic,strong) NSString * firstVideoImgUrl;

@end

@implementation DraftEditVideoViewController
...
- (void)setupFirst {
    ...
    if (NotNilAndNull(dic) && [dic.allKeys containsObject:@"route"]) {
        ...
        NSString *time = [self getVideoTimeByUrlString:urlStr];
        
        self.firstVideoUrl = urlStr;
        self.firstVideoImgUrl = imageUrl;
        self.firstVideoTime = time;
        
        [self.topVideoImageV sd_setImageWithURL:[NSURL URLWithString:self.firstVideoImgUrl] placeholderImage:[UIImage imageNamed:@"default_big"]];
    }
}
...
- (void)creatZFPlayer {
    ...
    if (self.selectIndexPath.row == 0) {
        self.player.assetURL = [NSURL URLWithString:self.firstVideoUrl];
    }else {
...
{
    AVURLAsset *urlAsset = (AVURLAsset *)self.currentAssetModel.videoAsset;
    if (self.selectIndexPath.row != 0 && (!urlAsset || ![urlAsset isKindOfClass:[AVURLAsset class]])) {
...
//    }
    ...
    DrafrPushVideoViewController * videoVC = [[DrafrPushVideoViewController alloc] init];
        
    if (self.selectIndexPath.row != 0) {
//        videoVC.isLocal = YES;
        videoVC.slAssetModel = self.currentAssetModel;
        videoVC.videoUrl = urlAsset.URL;
    }else {
        videoVC.videoImage = self.topVideoImageV.image;
        videoVC.videoUrl = [NSURL URLWithString:self.firstVideoUrl];
    }
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (NotNilAndNull(self.firstVideoUrl)) {
        return self.slAssetModelList.count + 1;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.localIcon.hidden = YES;
    if (self.firstVideoUrl && indexPath.row == 0) {
        cell.localIcon.hidden = NO;
        cell.timeLabel.text = @"";
//        cell.timeLabel.text = self.firstVideoTime;
        [cell.imageV sd_setImageWithURL:[NSURL URLWithString:self.firstVideoImgUrl] placeholderImage:[UIImage imageNamed:@"default_big"]];
    } else {
        SLAssetModel * assetModel = self.slAssetModelList[indexPath.row - 1];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        [self setupFirst];
        NSURL * videoUrl = [NSURL URLWithString:self.firstVideoUrl];
//        self.pushUrl = videoUrl;
//        self.pushImage = self.oneVideoImage.image;
        self.player.assetURL = videoUrl;
    }else {
        self.currentAssetModel = self.slAssetModelList[indexPath.row - 1];
...
- (UIImageView *)topVideoImageV {
    if (!_topVideoImageV) {
        ...
        _topVideoImageV.contentMode = UIViewContentModeScaleAspectFill;
