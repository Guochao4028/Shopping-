...
- (NSAttributedString *)_getDetailWithModel:(EMConversation *)aConversation
{
    ...
    switch (messageBody.type) {
        ...
        case EMMessageBodyTypeImage:
            latestMessageTitle = SLLocalizedString(@"[图片]");
            break;
        case EMMessageBodyTypeVoice:
            latestMessageTitle = SLLocalizedString(@"[音频]");
            break;
        case EMMessageBodyTypeLocation:
            latestMessageTitle = SLLocalizedString(@"[位置]");
            break;
        case EMMessageBodyTypeVideo:
            latestMessageTitle = SLLocalizedString(@"[视频]");
            break;
        case EMMessageBodyTypeFile:
            latestMessageTitle = SLLocalizedString(@"[文件]");
            break;
        default:
            break;
    }
...
//    }
    ...
    if (ext && [ext[kConversation_IsRead] intValue] == kConversation_AtAll) {
        NSString *allMsg = SLLocalizedString(@"[有全体消息]");
        ...
        NSString *atStr = SLLocalizedString(@"[有人@我]");
...
{
    _model = model;
    NSLog(@"model.emModel.latestMessage.ext %@", model.emModel.lastReceivedMessage.ext);
    
    EMConversation *conversation = model.emModel;
    if (conversation.type == EMConversationTypeChat) {
        self.avatarView.image = [UIImage imageNamed:@"navigationBar_leftIcon"];
    } else {
        self.avatarView.image = [UIImage imageNamed:@"navigationBar_leftIcon"];
    }

    if ([model.emModel.lastReceivedMessage.ext isKindOfClass:[NSDictionary class]] == YES) {
        
        NSDictionary *dic = model.emModel.lastReceivedMessage.ext;
        
        self.nameLabel.text = dic[@"name"];
        [self.avatarView sd_setImageWithURL:dic[@"headUrl"] placeholderImage:[UIImage imageNamed:@"navigationBar_leftIcon"]];
    }else{
