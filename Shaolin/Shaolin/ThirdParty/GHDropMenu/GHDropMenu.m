...
//        dropMenu.titles = [NSKeyedUnarchiver unarchivedObjectOfClass:[NSMutableArray class] fromData:newData error:nil];
//    } else {
        // Fallback on earlier versions
    dropMenu.titles =  [NSKeyedUnarchiver unarchiveObjectWithFile:filePath];
//    }
//     NSLog(@"%@",fooString);
...
- (instancetype)new {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:SLLocalizedString(@"请使用方法 creatDropMenuWithConfiguration: or creatDropFilterMenuWidthConfiguration: 代替初始化") userInfo:nil];
}
- (instancetype)init {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:SLLocalizedString(@"请使用方法 creatDropMenuWithConfiguration: or creatDropFilterMenuWidthConfiguration: 代替初始化") userInfo:nil];
}
...
- (void)dismiss {
    ...
    [UIView animateWithDuration:self.durationTime animations:^{
        if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle /** 普通菜单 */) {
            ...
            self.titleCover.frame = CGRectMake(0, self.tableY, kGHScreenWidth, 0);
            self.sure.frame = CGRectMake(0, CGRectGetMaxY(self.filter.frame)+9, 88, 30);
...
- (void)show {
    ...
    if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle /** 筛选菜单 */) {
        ...
        self.sure.frame = CGRectMake(kGHScreenWidth * .5, CGRectGetMaxY(self.filter.frame) +9, 88, 30);
        self.sure.alpha = 1;
        self.reset.alpha = 1;
...
//        self.filter.frame = CGRectMake(kGHScreenWidth * 0.1, 0, kGHScreenWidth * 0.9, kGHScreenHeight - kFilterButtonHeight - kGHSafeAreaBottomHeight);
        ...
        self.filter.frame = CGRectMake(kGHScreenWidth * 0.25, 0, kGHScreenWidth * 0.75, ScreenHeight - self.bottom);
        
//        self.filter.frame = CGRectMake(kGHScreenWidth * 0.1, self.top, kGHScreenWidth * 0.9, kGHScreenHeight - kFilterButtonHeight - kGHSafeAreaBottomHeight - self.top - self.bottom);
        [self.filterCover addSubview:self.bottomView];
        [self.bottomView addSubview:self.sure];
        [self.bottomView addSubview:self.reset];
        
//        [self.bottomView setFrame:CGRectMake(self.filter.x, CGRectGetMaxY(self.filter.frame), self.filter.width, kFilterButtonHeight)];
        
        
        [self.bottomView setFrame:CGRectMake(self.filter.x, CGRectGetMaxY(self.filter.frame), self.filter.width, ScreenHeight - CGRectGetMaxY(self.filter.frame) )];
        
        self.reset.frame = CGRectMake(27, 9.5, 88, 30);
        
        CGFloat x = CGRectGetWidth(self.filter.frame) - (27*2) - 60;
        
        self.sure.frame = CGRectMake(x, 9.5, 88, 30);
...
//                self.filterCover.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:102.0/255];
                [self.filterCover setBackgroundColor:[UIColor colorWithWhite:0 alpha:0.6]];
//                self.filterCover.backgroundColor = [UIColor clearColor];
...
- (void)dropMenuFilterEndInputItem:(GHDropMenuFilterInputItem *)item dropMenuModel:(GHDropMenuModel *)dropMenuModel {
    if (dropMenuModel.minPrice.length && dropMenuModel.maxPrice.length) {
        if (dropMenuModel.minPrice.doubleValue > dropMenuModel.maxPrice.doubleValue) {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:SLLocalizedString(@"提示") message:NSLocalizedString(@"最小价格不能大于最大价格,请重新选择", nil) delegate:nil cancelButtonTitle:SLLocalizedString(@"我知道了") otherButtonTitles:nil, nil];
            [alert show];
...
- (void)dropMenuFilterTimeChoseItem:(GHDropMenuFilterTimeChoseItem *)item type:(GHDropMenuFilterTimeChoseItemType)type dropMenuModel:(GHDropMenuModel *)dropMenuModel {
    ...
    alert.alertTitle = type == GHDropMenuFilterTimeChoseItemTypeBeginTime ?SLLocalizedString(@"选择开始时间"):SLLocalizedString(@"选择结束时间");
...
//            if([dropMenuSectionModel.sectionHeaderTitle isEqualToString:SLLocalizedString(@"品阶")]||[dropMenuSectionModel.sectionHeaderTitle isEqualToString:SLLocalizedString(@"活动状态")]){
//                return CGSizeMake((dropMenuModel.menuWidth - (dropMenuModel.sectionCount + 1) * 10) /dropMenuModel.sectionCount , 30.01f);
//            }else{
//
                CGFloat filterWidth = CGRectGetWidth(self.filter.frame);
                
                CGFloat width = (filterWidth - 32 - (11.5 * 2)) / 3;
                
                return CGSizeMake(width , 30.01f);
//            }
...
//
            return dropMenuSectionModel.sectionSeleted ? dropMenuSectionModel.dataArray.count:0;
        } else if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeInput ||
                   dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeSingleInput) {
            return 1;
        } else {
...
//           if (@available(iOS 11.0, *)) {
//               NSData *data = [NSKeyedArchiver archivedDataWithRootObject:self.titles requiringSecureCoding:YES error:nil];
//               [data writeToFile:filePath options:NSDataWritingAtomic error:nil];
//
//           } else {
//               // Fallback on earlier versions
               [NSKeyedArchiver archiveRootObject:self.titles  toFile:filePath];
//           }
...
- (UIView *)bottomView {
    if (_bottomView == nil) {
        ...
        CGFloat y = self.filter.frame.size.height + self.filter.frame.origin.y + kFilterButtonHeight;
        
        _bottomView.backgroundColor = [UIColor whiteColor];
//        _bottomView.frame = CGRectMake(self.filter.frame.origin.x, y,self.filter.frame.size.width, kGHSafeAreaBottomHeight);
        
        
         _bottomView.frame = CGRectMake(self.filter.frame.origin.x, y,self.filter.frame.size.width, ScreenHeight);
...
- (UIButton *)reset {
    if (_reset == nil) {
        ...
        [_reset setTitle:SLLocalizedString(@"重置") forState:UIControlStateNormal];
        [_reset.titleLabel setFont:kRegular(14)];
...
- (UIButton *)sure {
    if (_sure == nil) {
        ...
        [_sure addTarget:self action:@selector(clickButton:) forControlEvents:UIControlEventTouchUpInside];
         [_sure.titleLabel setFont:kRegular(14)];
        _sure.layer.cornerRadius = 15;
        _sure.alpha = 0;
    }
    return _sure;
}
...
//        _filter = [[UICollectionView alloc]initWithFrame:CGRectMake(kGHScreenWidth * 0.1, 0, kGHScreenWidth * 0.9, kGHScreenHeight - kFilterButtonHeight - kGHSafeAreaBottomHeight) collectionViewLayout:self.filterFlowLayout];
         ...
         _filter = [[UICollectionView alloc]initWithFrame:CGRectMake(kGHScreenWidth * 0.3, 0, kGHScreenWidth * 0.7, ScreenHeight) collectionViewLayout:self.filterFlowLayout];
