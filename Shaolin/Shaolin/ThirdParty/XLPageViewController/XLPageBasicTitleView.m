...
- (void)fixShadowLineCenter {
    ...
    /*
     检查一下总页数，避免尝试滚动到无效的索引路径引发的崩溃
     attempt to scroll to invalid index path
     */
    NSInteger allPage = [self.dataSource pageTitleViewNumberOfTitle];
    if (allPage < _selectedIndex || allPage == 0) return;
    //避免找不到Cell,先滚动到指定位置
    ...
    [self resetShadowLineCenter];
}

- (void)resetShadowLineCenter{
    static int count = 0;
    NSLog(@"resetShadowLineCenter count : %d", count);
    CGPoint shadowCenter = [self shadowLineCenterForIndex:_selectedIndex];
    if (shadowCenter.x > 0) {
        self.shadowLine.center = shadowCenter;
        count = 0;
    }else {
        if (shadowCenter.x <= 0 && count < 3) { //避免cell不在屏幕上显示，延时0.1秒加载，并最大重试3次
            WEAKSELF
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
                [weakSelf resetShadowLineCenter];
                count++;
            });
        } else {
            self.shadowLine.center = shadowCenter;
...
- (CGPoint)shadowLineCenterForIndex:(NSInteger)index {
    ...
    NSLog(@"shadowLineCenter(%.1f, %.1f)", centerX, centerY);
