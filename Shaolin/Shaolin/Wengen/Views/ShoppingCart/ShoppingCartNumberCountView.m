...
//    NSString *text = self.numberLabel.text;
    ...
    if (self.checkType == CheckInventoryGoodsType) {
        [dic setValue:self.goodsModel.goods_id forKey:@"goods_id"];
        
        if(self.goodsModel.goods_attr_id != nil && self.goodsModel.goods_attr_id.length > 0){
            [dic setValue:self.goodsModel.goods_attr_id forKey:@"goods_attr_id"];
...
//        [_numberLabel setBackgroundColor:[UIColor colorForHex:@"F3F3F3"]];
//
//
//
//    }
...
//        _numberTextField = [[UITextField alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.subButton.frame)+7.5, 0, 20*1.5, 20)];
        ...
        [_numberTextField setBackgroundColor:[UIColor colorForHex:@"F3F3F3"]];
