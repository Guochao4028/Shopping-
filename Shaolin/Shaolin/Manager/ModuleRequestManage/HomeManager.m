...
- (void)postSubmitPhotoWithFileData:(NSData *)fileData
                            ...
                            failure:(SLFailureReasonBlock)failure
                             finish:(SLFinishedResultBlock)finish{
    NSString *url = Found_POST_Photo;
            
           
    [SLRequest uploadRequestWithApi:url isVideo:isVedio fileData:fileData progress:^(double progress) {
        
    } success:success failure:failure finish:finish];
...
//     NSString *url = [NSString stringWithFormat:@"%@%@",Found,Found_POST_WebNewsInforMation];
     ...
     NSString *url = Found_POST_WebNewsInforMation;
     NSDictionary *params = @{
        @"title":title,
...
-(void)postSharedContentId:(NSString *)contentId
    ...
    NSString *url = Foune_POST_DetailsPraise;
    NSDictionary *params;
    if ([type isEqualToString:@"4"]) {
        params = @{
            @"pujaCode":contentId,
...
-(void)postCollectionContentId:(NSString *)contentId
                    ...
                    MemberName:(NSString *)memberName
                       ...
                       failure:(SLFailureReasonBlock)failure
                        finish:(SLFinishedResultBlock)finish{
    NSString *url = Foune_POST_DetailsCollection;
       NSDictionary *params;
       if ([type isEqualToString:@"4"]) {
           params = @{
               @"pujaCode":contentId,
...
- (void)getArticleDetails:(NSString *)articleId
    ...
    NSString *url = @"";
       ...
       if ([tabbarStr isEqualToString:@"Found"]){
           url = Foune_Get_ArticleDetails;
       } else { //Activity
           url =  Activity_GET_ArticleDetails;
       }
       NSLog(@"%@ - 文章详情 URL%@", tabbarStr, url);
       NSLog(@"%@ - 文章详情 参数%@", tabbarStr, params);
    [SLRequest getRequestWithApi:url parameters:params success:success failure:failure finish:finish];
