...
@interface PersonDataEditVc ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIGestureRecognizerDelegate,TZImagePickerControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,GCTextFieldDelegate>
...
@property(nonatomic,strong) GCTextField *nickNameTf;//昵称
...
@property(nonatomic,strong) GCTextField *sigNameTf;//签名
...
- (void)getData {
    ...
    if ([[self.dicData objectForKey:@"gender"] integerValue]== 0 ) {
        ...
        [self.sexBtn setTitleColor:KTextGray_B7 forState:(UIControlStateNormal)];
    }else{
...
- (void) bottomRightBtnHandle
{
    if (self.pickerType == 1) {
        ...
        [self.birthdayBtn setTitleColor:KTextGray_12 forState:(UIControlStateNormal)];
...
- (UITextField *)nickNameTf {
    if (!_nickNameTf) {
        _nickNameTf = [[GCTextField alloc] initWithFrame:CGRectMake(SLChange(92),0, kWidth -SLChange(112), SLChange(60))];
        _nickNameTf.inputType = CCCheckNone;
        _nickNameTf.maxLimit = 20;
        [_nickNameTf setTextColor:KTextGray_12];
        ...
        [_nickNameTf setValue:KTextGray_B7  forKeyPath:@"placeholderLabel.textColor"];
...
- (UITextField *)sigNameTf {
    if (!_sigNameTf) {
        _sigNameTf = [[GCTextField alloc] initWithFrame:CGRectMake(SLChange(92),0, kWidth -SLChange(112), SLChange(60))];
        _sigNameTf.maxLimit = 40;
        
        [_sigNameTf setTextColor:KTextGray_12];
        ...
        [_sigNameTf setValue:KTextGray_B7  forKeyPath:@"placeholderLabel.textColor"];
...
- (UIButton *)caseBtn {
    if (!_caseBtn) {
        ...
        [_caseBtn setTitleColor:UIColor.whiteColor forState:(UIControlStateNormal)];
...
- (void)caseAction {
    ...
    if (self.nickNameTf.text.length == 0) {
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"请输入昵称") view:self.view afterDelay:TipSeconds];
        return;
    }
...
//    }
    if (self.emailTf.text.length > 0){
        ...
        if (!email) {
            [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"邮箱格式不正确") view:self.view afterDelay:TipSeconds];
            return;
        }
    }
...
- (SLDatePickerView *)birthPickerView {
    ...
    if (!_birthPickerView) {
        ...
        _birthPickerView.pickerStyle.titleTextColor = KTextGray_12;
        // 2.设置属性
        ...
        _birthPickerView.resultBlock = ^(NSDate *selectDate, NSString *selectValue) {
            ...
            [weakSelf.birthdayBtn setTitleColor:KTextGray_12 forState:(UIControlStateNormal)];
...
//        _sexPickerView.pickerStyle = [BRPickerStyle pickerStyleWithDoneBtnImage:[UIImage imageNamed:@"goodsClose"]];
        ...
        _sexPickerView.pickerStyle.titleTextColor = KTextGray_12;
...
//        };
        _sexPickerView.resultModelBlock = ^(BRResultModel * _Nullable resultModel) {
            ...
            [weakSelf.sexBtn setTitleColor:KTextGray_12 forState:(UIControlStateNormal)];
        };
//        _sexPickerView.resultModelBlock = ^(BRResultModel * _Nullable resultModel) {
//
//            weakSelf.tempSexStr = resultModel.value;
...
- (UIButton *)sexBtn {
    if (!_sexBtn) {
        ...
        [_sexBtn setTitleColor:KTextGray_12 forState:(UIControlStateNormal)];
...
- (UIButton *)birthdayBtn {
    if (!_birthdayBtn) {
        ...
        [_birthdayBtn setTitleColor:KTextGray_12 forState:(UIControlStateNormal)];
