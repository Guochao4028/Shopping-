...
- (void)layoutSubviews {
    ...
    if (self.cellStyle == Authentication_Passport){
        ...
        self.imageView.image = [UIImage imageNamed:@"Authentication_Passport"];
    } else { //实人认证
        self.titleLabel.text = SLLocalizedString(@"身份证认证");
        self.tipsLabel.text = SLLocalizedString(@"安全、方便快捷");
        self.recommendLabel.hidden = NO;
        self.imageView.image = [UIImage imageNamed:@"Authentication_Person"];
    }
...
- (UILabel *)tipsLabel{
    if (!_tipsLabel){
        ...
        _tipsLabel.textColor = [UIColor colorForHex:@"999999"];
    }
    return _tipsLabel;
}
...
- (UILabel *)verifiedStateLabel{
    if (!_verifiedStateLabel){
        ...
        _verifiedStateLabel.textColor = [UIColor colorForHex:@"999999"];
    }
