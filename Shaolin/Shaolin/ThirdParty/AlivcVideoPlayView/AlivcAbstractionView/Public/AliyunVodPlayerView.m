...
- (void)onTrackChanged:(AliPlayer*)player info:(AVPTrackInfo*)info {
    ...
    [self.controlView setBottomViewTrackInfo:info];
    NSString *showString = [NSString stringWithFormat:@"%@%@",SLLocalizedString(@"已为你切换至"),NSLocalizedString(info.trackDefinition, nil) ];
...
- (void)onPlayerStatusChanged:(AliPlayer*)player oldStatus:(AVPStatus)oldStatus newStatus:(AVPStatus)newStatus {
    ...
    if (newStatus == AVPStatusPaused) {
        playStatusString = SLLocalizedString(@"暂停");
    }
    if (newStatus == AVPStatusStarted) {
        playStatusString = SLLocalizedString(@"播放");
    }
...
- (void)showPopLayerWithErrorModel:(AVPErrorModel *)errorModel {
    NSString *errorShowMsg = [NSString stringWithFormat:SLLocalizedString(@"%@\n错误码:%d"),errorModel.message,(int)errorModel.code];
...
- (void)setUIStatusToReplay{
    [self.popLayer showPopViewWithCode:ALYPVPlayerPopCodeUseMobileNetwork  popMsg:SLLocalizedString(@"视频播放完成")];
}
