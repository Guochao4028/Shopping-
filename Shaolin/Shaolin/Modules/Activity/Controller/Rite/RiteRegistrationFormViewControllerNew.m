...
- (FormViewModel *)createTVModel:(NSString *)identifier title:(NSString *)title value:(NSString *)value placeholder:(NSString *)placeholder checkType:(CCCheckType)checkType params:(NSDictionary *)params{
    FormViewModel *model = [FormViewModel textViewModel:FormViewLayoutStyle_Vertical identifier:identifier title:title checkType:checkType placeholder:placeholder];
    model.value = value;
    if (params){
        [model.params addEntriesFromDictionary:params];
    }
...
- (FormView *)formView{
    if (!_formView){
        ...
        _formView.secondBackColor = [UIColor whiteColor];//KTextGray_EEE;
...
- (UIView *)bottomView{
    if (!_bottomView){
        ...
        self.tipsLabel.textColor = KTextGray_666;
