...
//    [self cornerRadius:self.detailedView radius:4];
    ...
    if (self.isCollect == YES) {
           ...
           self.focusButton.layer.cornerRadius = 30/2;
           [self.focusButton setTitle:SLLocalizedString(@"已关注") forState:UIControlStateNormal];
           [self.focusButton setTitleColor:KTextGray_999 forState:UIControlStateNormal];
       }else{
          [self.focusButton setImage:[UIImage imageNamed:@"baiGuanzhu"] forState:UIControlStateNormal];
           ...
           [self.focusButton setBackgroundColor:kMainYellow];
           self.focusButton.layer.borderColor = kMainYellow.CGColor;
           self.focusButton.layer.borderWidth = 1;
           self.focusButton.layer.cornerRadius = 30/2;
    ...
    if (sender.isSelected) {
        ...
        [[DataManager shareInstance]addCollect:@{@"clubId":self.storeId, @"type":@"1"} Callback:^(Message *message) {
            ...
            if (message.isSuccess == YES) {
                ...
                [sender setTitleColor:KTextGray_999 forState:UIControlStateNormal];
