...
- (void)layoutView:(NSString *)title
{
    ...
    if ([title isEqualToString:SLLocalizedString(@"注册账号")]) {
        [self.registBtn setTitle:SLLocalizedString(@"注册") forState:(UIControlStateNormal)];
    }else
    {
         _passWordTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SLLocalizedString(@"请重置密码") attributes:@{NSForegroundColorAttributeName: KTextGray_96}];
...
- (UIButton *)closeBtn
{
    if (!_closeBtn) {
        ...
        [_closeBtn setImage:[UIImage imageNamed:@"new_regist_close"] forState:(UIControlStateNormal)];
...
- (UITextField *)phoneTextField
{
    if (!_phoneTextField) {
        _phoneTextField = [[UITextField alloc] init];
                _phoneTextField.textColor = KTextGray_333;
        ...
        [userNameClearButton setImage:[UIImage imageNamed:@"new_forget_clear"] forState:UIControlStateNormal];
                _phoneTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SLLocalizedString(@"请输入手机号") attributes:@{NSForegroundColorAttributeName: KTextGray_96}];
...
- (UITextField *)codeTextField
{
    if (!_codeTextField) {
        _codeTextField = [[UITextField alloc] init];
                _codeTextField.textColor = KTextGray_333;
                _codeTextField.secureTextEntry = NO;
                _codeTextField.keyboardType = UIKeyboardTypeNumberPad;
                if (@available(iOS 12.0, *)) {
                        _codeTextField.textContentType = UITextContentTypeOneTimeCode;
                }
                _codeTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SLLocalizedString(@"请输入验证码") attributes:@{NSForegroundColorAttributeName: KTextGray_96}];
...
- (UITextField *)passWordTextField
{
    if (!_passWordTextField) {
        _passWordTextField = [[UITextField alloc] init];
                _passWordTextField.textColor = KTextGray_333;
...
//                _passWordTextField.keyboardType = UIKeyboardTypeDefault;
                ...
                _passWordTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SLLocalizedString(@"请输入8-16位密码") attributes:@{NSForegroundColorAttributeName: KTextGray_96}];
...
- (UIButton *)lookBtn
{
    if (!_lookBtn) {
        ...
        [_lookBtn setImage:[UIImage imageNamed:@"new_password_close"] forState:(UIControlStateNormal)];
         [_lookBtn setImage:[UIImage imageNamed:@"new_password_open"] forState:(UIControlStateSelected)];
...
//        [_registBtn setBackgroundImage:[UIImage imageNamed:@"regist_highlighted"] forState:(UIControlStateHighlighted)];
        _registBtn.backgroundColor = kMainYellow;
        _registBtn.cornerRadius = SLChange(45)/2;
        [_registBtn setTitleColor:KTextGray_F1 forState:(UIControlStateNormal)];
...
- (void)tapVerBtnAction:(id)sender
{
    ...
    if (self.phoneTextField.text.length == 0) {
        ...
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"请输入正确的手机号") view:self afterDelay:TipSeconds];
        return;
    }
...
- (void)registAction:(UIButton *)btn
{
    ...
    if (self.codeTextField.text.length == 0) {
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"请输入验证码") view:self afterDelay:TipSeconds];
        return;
    }
    if (self.codeTextField.text.length < 4){
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"请输入正确的验证码") view:self afterDelay:TipSeconds];
        return;
    }
    if (self.passWordTextField.text.length == 0){
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"请填写密码") view:self afterDelay:TipSeconds];
        return;
    } else if (self.passWordTextField.text.length < 8 || self.passWordTextField.text.length > 16 ) {
