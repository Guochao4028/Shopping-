...
/**
 * 功能：播放器播放结束，文字描述；
 * 参数：默认文案：SLLocalizedString(@"再次观看，请点击重新播放");
 */
- (void)setPlayFinishDescribe:(NSString *)des;

/**
 * 功能：播放器网络状态不好，文字描述；
 * 参数：默认文案：SLLocalizedString(@"当前网络不佳，请稍后点击重新播放");
 */
- (void)setNetTimeOutDescribe:(NSString *)des;

/**
 * 功能：播放器无网络，文字描述；
 * 参数：默认文案：SLLocalizedString(@"无网络连接，检查网络后点击重新播放");
 */
- (void)setNoNetDescribe:(NSString *)des;

/**
 * 功能：播放器加载出错，文字描述；
 * 参数：默认文案：SLLocalizedString(@"视频加载出错，请点击重新播放");
 */
- (void)setLoaddataErrorDescribe:(NSString *)des;

/**
 * 功能：播放器4G网络状态，文字描述；
 * 参数：默认文案：SLLocalizedString(@"当前为移动网络，请点击播放");
 */
