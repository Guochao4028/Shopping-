...
- (void)_setupSubviews
{
    ...
    if (self.canSend) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:SLLocalizedString(@"发送") style:UIBarButtonItemStylePlain target:self action:@selector(sendAction)];
    }
    self.title = SLLocalizedString(@"地理位置");
...
- (void)_startLocation
{
    ...
    [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"正在定位...") view:self.view afterDelay:TipSeconds];
}
...
//    [self hideHud];
    if (error.code == 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:[error.userInfo objectForKey:NSLocalizedRecoverySuggestionErrorKey] delegate:nil cancelButtonTitle:SLLocalizedString(@"确定") otherButtonTitles:nil, nil];
        [alertView show];
    }
}
