...
- (UIButton *)backBtn{
    if (!_backBtn) {
        ...
        UIImage *image = [UIImage imageNamed:@"video_left"];
        [_backBtn setImage:image forState:UIControlStateNormal];
        [_backBtn setImage:image forState:UIControlStateHighlighted];
        _backBtn.hidden = YES;
    }
    return _backBtn;
}
...
/*
 #define ALIYUNVODVIEW_UNKNOWN              SLLocalizedString(@"未知错误")
 #define ALIYUNVODVIEW_PLAYFINISH           SLLocalizedString(@"再次观看，请点击重新播放")
 #define ALIYUNVODVIEW_NETWORKTIMEOUT       SLLocalizedString(@"当前网络不佳，请稍后点击重新播放")
 #define ALIYUNVODVIEW_NETWORKUNREACHABLE   SLLocalizedString(@"无网络连接，检查网络后点击重新播放")
 #define ALIYUNVODVIEW_LOADINGDATAERROR     SLLocalizedString(@"视频加载出错，请点击重新播放")
 #define ALIYUNVODVIEW_USEMOBILENETWORK     SLLocalizedString(@"当前为移动网络，请点击播放")
 */
