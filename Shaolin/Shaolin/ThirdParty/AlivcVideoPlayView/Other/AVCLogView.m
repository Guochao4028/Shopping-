...
+ (NSString *)descriptionWithEvent:(AVPEventType)event{
    NSString *eventString = @"";
    switch (event) {
        case AVPEventPrepareDone:
            eventString = SLLocalizedString(@"准备成功");
            break;
        case AVPEventAutoPlayStart:
            eventString = SLLocalizedString(@"开始播放");
            break;
        case AVPEventFirstRenderedStart:
            eventString = SLLocalizedString(@"第一帧加载完成");
            break;
        case AVPEventCompletion:
            eventString = SLLocalizedString(@"播放已结束");
            break;
        case AVPEventLoadingStart:
            eventString = SLLocalizedString(@"开始加载视频数据");
            break;
        case AVPEventLoadingEnd:
            eventString = SLLocalizedString(@"视频资源加载完成");
            break;
        case AVPEventSeekEnd:
            eventString = SLLocalizedString(@"跳转成功");
            break;
        case AVPEventLoopingStart:
            eventString = SLLocalizedString(@"循环播放开始事件");
            break;            
        default:
            break;
    }
...
- (instancetype)initWithEvent:(AVPEventType)event{
    self = [super init];
    if (self) {
...
- (void)configBaseUI{
    ...
    deleteLabel.text = [SLLocalizedString(@"清除日志") localString];
