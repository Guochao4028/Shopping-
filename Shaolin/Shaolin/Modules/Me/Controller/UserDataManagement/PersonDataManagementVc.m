...
- (void)viewDidLoad {
    [super viewDidLoad];
    _levelLabelTag = 200;
    self.dataArray = @[@[SLLocalizedString(@"修改密码"),SLLocalizedString(@"实名认证"),/*SLLocalizedString(@"支付密码"),*/ SLLocalizedString(@"三方登录")], @[SLLocalizedString(@"收货地址"), SLLocalizedString(@"增票资质"), SLLocalizedString(@"清除缓存"),SLLocalizedString(@"关于少林")]];
...
- (void)getData {
    ...
    if (IsNilOrNull(autograph) || autograph.length == 0) {
        ...
        self.signatureLabel.text = [self.dicData objectForKey:@"autograph"];
    }
...
//        [self.levelView setGradationColor:colors startPoint:CGPointMake(0.0, 0.0) endPoint:CGPointMake(1.0, 1.0)];
        ...
        levelLabel.textColor = KTextGray_12;// [UIColor colorForHex:@"FFFFFF"];
        self.levelView.backgroundColor = KMeLightYellow;//[UIColor colorForHex:@"DAB477"];
    }
}
...
-(void)layoutView
{
    ...
    [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(-SLChange(30));
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
     ...
     if (!cell)
     {
        cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:cellIdentifier];
         ...
         [line setBackgroundColor:KTextGray_FA];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *title = self.dataArray[indexPath.section][indexPath.row];
    if ([title isEqualToString:SLLocalizedString(@"修改密码")]){
...
//                return;
    ...
    } else if ([title isEqualToString:SLLocalizedString(@"支付密码")]){
        PayPasswordVc * v = [PayPasswordVc new];
        [self.navigationController pushViewController:v animated:YES];
    } else if ([title isEqualToString:SLLocalizedString(@"三方登录")]){
        WEAKSELF
    ...
    } else if ([title isEqualToString:SLLocalizedString(@"收货地址")]){
        AddressViewController *addressVC = [[AddressViewController alloc]init];
        [self.navigationController pushViewController:addressVC animated:YES];
    } else if ([title isEqualToString:SLLocalizedString(@"增票资质")]){
    ...
    } else if ([title isEqualToString:SLLocalizedString(@"关于少林")]){
...
//        _idLabel.textColor = [UIColor colorForHex:@"999999"];
        _idLabel.textColor = KTextGray_12;
        _idLabel.text = @"";
...
//        _signatureLabel.textColor = [UIColor colorForHex:@"737373"];
        _signatureLabel.textColor = KTextGray_12;
...
//        [_outBtn setBackgroundImage:[UIImage imageNamed:@"outLogin"] forState:(UIControlStateNormal)];
        [_outBtn setBackgroundColor:kMainYellow];
