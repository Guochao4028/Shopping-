...
- (void)reloadPhoneNumberAndPassword{
    ...
    BOOL savePassword = [[[NSUserDefaults standardUserDefaults] objectForKey:@"savePassword"] boolValue];
    ...
    [self.rememberPassWordBtn setSelected:savePassword];
...
- (void)rememberAction:(UIButton *)button
{
    ...
    {
        self.selectPassWord = 0;
        [button setTitleColor:KTextGray_96 forState:(UIControlStateNormal)];
    }
}
...
- (void)setUserDefaults:(NSString *)phoneNumber passWord:(NSString *)passWord loginType:(NSString *)loginType{
    BOOL savePassword = self.rememberPassWordBtn.selected;
    [[NSUserDefaults standardUserDefaults] setBool:self.rememberPassWordBtn.selected forKey:@"savePassword"];
    // 当前未选择记住密码，则不将密码存入本地
...
- (UIButton *)rememberPassWordBtn
{
    if (!_rememberPassWordBtn) {
        ...
        [_rememberPassWordBtn setImage:[UIImage imageNamed:@"new_login_remember_choose"] forState:(UIControlStateSelected)];
        ...
        [_rememberPassWordBtn setTitleColor:KTextGray_96 forState:(UIControlStateNormal)];
...
- (UIButton *)loginBtn
{
    if (!_loginBtn) {
        ...
        [_loginBtn setTitleColor:KTextGray_F1 forState:(UIControlStateNormal)];
        [_loginBtn setBackgroundColor:kMainYellow];
        _loginBtn.cornerRadius = SLChange(45)/2;
...
- (UIView *)viewLine
{
    if (!_viewLine) {
        ...
        _viewLine.backgroundColor = kMainYellow;
    }
    return _viewLine;
}
- (UIView *)crossView1
{
    if (!_crossView1) {
        ...
        _crossView1.backgroundColor = kMainYellow;
    }
    return _crossView1;
}
- (UIView *)crossView2
{
    if (!_crossView2) {
        ...
        _crossView2.backgroundColor = kMainYellow;
    }
    return _crossView2;
}
- (UILabel *)thirdpartyTitleLabe
{
    if (!_thirdpartyTitleLabe) {
        ...
        _thirdpartyTitleLabe.textColor = kMainYellow;
...
- (UIButton *)agreementBtn
{
    if (!_agreementBtn) {
        ...
        [_agreementBtn setImage:[UIImage imageNamed:@"new_login_remember_choose"] forState:(UIControlStateSelected)];
...
- (YYLabel *)agreementYYLabel {
    if (!_agreementYYLabel) {
        ...
        [string addAttributes:@{NSForegroundColorAttributeName: KTextGray_333} range:NSMakeRange(0, agreementRange.location)];
        [string addAttributes:@{NSForegroundColorAttributeName: redColor} range:agreementRange];
        [string addAttributes:@{NSForegroundColorAttributeName: KTextGray_333} range:NSMakeRange(NSMaxRange(agreementRange), privacyRange.location - NSMaxRange(agreementRange))];
