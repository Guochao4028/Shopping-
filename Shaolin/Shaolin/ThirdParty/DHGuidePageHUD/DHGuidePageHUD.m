...
//        [skipButton setTitle:SLLocalizedString(@"跳过") forState:UIControlStateNormal];
...
//        [self addSubview:skipButton];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIView *skipBgView = [[UIView alloc]initWithFrame:CGRectMake(0, frame.size.height - 43, frame.size.width, 43)];
                skipBgView.backgroundColor = UIColor.whiteColor;
...
//                [startButton setTitle:SLLocalizedString(@"开始体验") forState:UIControlStateNormal];
...
- (instancetype)dh_initWithFrame:(CGRect)frame videoURL:(NSURL *)videoURL {
    if ([super initWithFrame:frame]) {
        ...
        [movieStartButton setTitle:SLLocalizedString(@"开始体验") forState:UIControlStateNormal];
