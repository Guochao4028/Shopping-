...
- (void)_setupNavigationBarTitle
{
    ...
    /**2020-07-18  通过扩展 设置名称 GC*/
    if ([self.conversationModel.emModel.lastReceivedMessage.ext isKindOfClass:[NSDictionary class]] == YES) {
        NSDictionary *dic = self.conversationModel.emModel.lastReceivedMessage.ext;
        
        self.titleLabel.text = dic[@"name"];
    }
    
//    self.titleLabel.text = SLLocalizedString(@"在线客服");
    ...
    self.tableView.y = 120;// CGRectGetMaxY(self.instructionsLabel.frame) +17;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        ...
        if (model.type == EMMessageTypeExtRecall) {
            cellString = SLLocalizedString(@"您撤回一条消息");
        }
    }
                    ...
                    if (asset) {
                        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData *data, NSString *uti, UIImageOrientation orientation, NSDictionary *dic){
                            if (data != nil) {
                                ...
                                [EMAlertController showErrorAlert:SLLocalizedString(@"图片太大，请选择其他图片")];
                            }
                        }];
                    }
                }];
...
- (void)chatBarDidCameraAction
{
    ...
    [EMAlertController showErrorAlert:SLLocalizedString(@"模拟器不支持照相机")];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            switch (status) {
                ...
                case PHAuthorizationStatusDenied: //用户已经明确否认了这一照片数据的应用程序访问
                    [EMAlertController showErrorAlert:SLLocalizedString(@"不允许访问相册")];
                    break;
                case PHAuthorizationStatusRestricted://此应用程序没有被授权访问的照片数据。可能是家长控制权限
                    [EMAlertController showErrorAlert:SLLocalizedString(@"没有授权访问相册")];
                    
                    break;
                    
                default:
                    [EMAlertController showErrorAlert:SLLocalizedString(@"访问相册失败")];
...
{
//    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:SLLocalizedString(@"实时通话类型") message:nil preferredStyle:UIAlertControllerStyleActionSheet];
//
//    __weak typeof(self) weakself = self;
//    if (self.conversationModel.emModel.type == EMConversationTypeChat) {
//        [alertController addAction:[UIAlertAction actionWithTitle:SLLocalizedString(@"语音通话") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
//        }]];
//        [alertController addAction:[UIAlertAction actionWithTitle:SLLocalizedString(@"视频通话") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//            [weakself.chatBar clearMoreViewAndSelectedButton];
...
//        }]];
//    } else {
//        [alertController addAction:[UIAlertAction actionWithTitle:SLLocalizedString(@"会议模式") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//            [weakself.chatBar clearMoreViewAndSelectedButton];
//
...
//        }]];
//        [alertController addAction:[UIAlertAction actionWithTitle:SLLocalizedString(@"直播模式") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//            [weakself.chatBar clearMoreViewAndSelectedButton];
//
...
//    }
//
//    [alertController addAction:[UIAlertAction actionWithTitle:SLLocalizedString(@"取消") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
//    }]];
//
...
- (void)_imageMessageCellDidSelected:(EMMessageCell *)aCell
{
    ...
    void (^downloadThumbBlock)(EMMessageModel *aModel) = ^(EMMessageModel *aModel) {
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"获取缩略图...") view:self.view afterDelay:TipSeconds];
    ...
    if (isCustomDownload) {
     ...
     MBProgressHUD *hud = [ShaolinProgressHUD defaultLoadingWithText:SLLocalizedString(@"下载原图...")];
    ...
    [[EMClient sharedClient].chatManager downloadMessageAttachment:aCell.model.emModel progress:nil completion:^(EMMessage *message, EMError *error) {
        ...
        if (error) {
            [EMAlertController showErrorAlert:SLLocalizedString(@"下载原图失败")];
            ...
            if (image) {
                ...
                [EMAlertController showErrorAlert:SLLocalizedString(@"获取原图失败")];
...
- (void)_audioMessageCellDidSelected:(EMMessageCell *)aCell
{
    ...
    if (body.downloadStatus == EMDownloadStatusDownloading) {
        [EMAlertController showInfoAlert:NSLocalizedString(@"正在下载语音,稍后点击", nil)];
...
//            [[EMCDDeviceManager sharedInstance] disableProximitySensor];
    ...
    MBProgressHUD *hud = [ShaolinProgressHUD defaultLoadingWithText:SLLocalizedString(@"下载语音...")];
    
    [[EMClient sharedClient].chatManager downloadMessageAttachment:aCell.model.emModel progress:nil completion:^(EMMessage *message, EMError *error) {
        [hud hideAnimated:YES];
        if (error) {
            [EMAlertController showErrorAlert:SLLocalizedString(@"下载语音失败")];
...
- (void)_videoMessageCellDidSelected:(EMMessageCell *)aCell
{
    ...
    if (body.thumbnailDownloadStatus == EMDownloadStatusFailed || ![fileManager fileExistsAtPath:body.thumbnailLocalPath]) {
        [ShaolinProgressHUD singleTextHud:SLLocalizedString(@"下载缩略图") view:self.view afterDelay:TipSeconds];
        if (!isCustomDownload) {
    ...
    if (body.downloadStatus == EMDownloadStatusDownloading) {
        [EMAlertController showInfoAlert:NSLocalizedString(@"正在下载视频,稍后点击", nil)];
    ...
    if (isCustomDownload) {
        ...
        MBProgressHUD *hud = [ShaolinProgressHUD defaultLoadingWithText:SLLocalizedString(@"下载视频...")];
        __weak typeof(self) weakself = self;
        [[EMClient sharedClient].chatManager downloadMessageAttachment:aCell.model.emModel progress:nil completion:^(EMMessage *message, EMError *error) {
            [hud hideAnimated:YES];
            if (error) {
                [EMAlertController showErrorAlert:SLLocalizedString(@"下载视频失败")];
                       ...
                       user:(NSString *)aUsername
{
    ...
    if (conversation.type == EMChatTypeChatRoom && [aChatroom.chatroomId isEqualToString:conversation.conversationId]) {
        NSString *str = [NSString stringWithFormat:SLLocalizedString(@"%@ 进入聊天室"), aUsername];
        [ShaolinProgressHUD singleTextHud:str view:self.view afterDelay:TipSeconds];
    }
}
                        ...
                        user:(NSString *)aUsername
{
    EMConversation *conversation = self.conversationModel.emModel;
    if (conversation.type == EMChatTypeChatRoom && [aChatroom.chatroomId isEqualToString:conversation.conversationId]) {
        NSString *str = [NSString stringWithFormat:SLLocalizedString(@"%@ 离开聊天室"), aUsername];
        [ShaolinProgressHUD singleTextHud:str view:self.view afterDelay:TipSeconds];
    }
}
...
- (void)_joinChatroom
{
    ...
    MBProgressHUD *hud = [ShaolinProgressHUD defaultLoadingWithText:SLLocalizedString(@"加入聊天室...")];
    [[EMClient sharedClient].roomManager joinChatroom:self.conversationModel.emModel.conversationId completion:^(EMChatroom *aChatroom, EMError *aError) {
        [hud hideAnimated:YES];
        if (aError) {
            [EMAlertController showErrorAlert:SLLocalizedString(@"加入聊天室失败")];
...
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"(づ｡◕‿‿◕｡)づ" message:SLLocalizedString(@"需要自定义实现上传附件方法") preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:SLLocalizedString(@"好的") style:UIAlertActionStyleCancel handler:nil];
...
- (UIMenuItem *)deleteMenuItem
{
    if (_deleteMenuItem == nil) {
        _deleteMenuItem = [[UIMenuItem alloc] initWithTitle:SLLocalizedString(@"删除") action:@selector(deleteMenuItemAction:)];
    }
...
- (UIMenuItem *)copyMenuItem
{
    if (_copyMenuItem == nil) {
        _copyMenuItem = [[UIMenuItem alloc] initWithTitle:SLLocalizedString(@"复制") action:@selector(copyMenuItemAction:)];
    }
...
//        _transpondMenuItem = [[UIMenuItem alloc] initWithTitle:SLLocalizedString(@"转发") action:@selector(transpondMenuItemAction:)];
//    }
//
...
- (UIMenuItem *)recallMenuItem
{
    if (_recallMenuItem == nil) {
        _recallMenuItem = [[UIMenuItem alloc] initWithTitle:SLLocalizedString(@"撤回") action:@selector(recallMenuItemAction:)];
    }
...
- (void)recallMenuItemAction:(UIMenuItem *)aItem
{
    ...
    [[EMClient sharedClient].chatManager recallMessage:model.emModel completion:^(EMMessage *aMessage, EMError *aError) {
        if (aError) {
            ...
            EMTextMessageBody *body = [[EMTextMessageBody alloc] initWithText:SLLocalizedString(@"您撤回一条消息")];
                 ...
                 completion:(void (^)(EMMessage *message))aCompletionBlock
{
    ...
    [[EMClient sharedClient].chatManager sendMessage:message progress:nil completion:^(EMMessage *message, EMError *error) {
        if (error) {
            ...
            [EMAlertController showErrorAlert:SLLocalizedString(@"转发消息失败")];
            
        } else {
            if (aCompletionBlock) {
                aCompletionBlock(message);
            }
            [EMAlertController showSuccessAlert:SLLocalizedString(@"转发消息成功")];
        }
    }];
}
...
-(UILabel *)instructionsLabel{
    ...
    if (_instructionsLabel == nil) {
        _instructionsLabel = [[UILabel alloc]initWithFrame:CGRectMake(62, 12, ScreenWidth - (62*2), 25)];
        
        [_instructionsLabel setFont:kRegular(14)];
        [_instructionsLabel setTextColor:[UIColor whiteColor]];
        [_instructionsLabel setBackgroundColor:[UIColor colorForHex:@"D8D8D8"]];
        [_instructionsLabel setText:SLLocalizedString(@"您好，少林智能客服助手为您服务！")];
        [_instructionsLabel setTextAlignment:NSTextAlignmentCenter];
        _instructionsLabel.layer.masksToBounds = YES;
        _instructionsLabel.layer.cornerRadius = 12.5;
