    ...
    if (method == lgf_GET) {
        ...
        task = [[LGFNetwork lgf_Once] lgf_GET:url parameters:paramt success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
            ...
            if (error.code == -1001 || [error.localizedDescription isEqualToString:@"The request timed out."]) {
                NSError *err = [NSError errorWithDomain:@"LGF" code: -1001 userInfo:@{NSLocalizedDescriptionKey:SLLocalizedString(@"当前网络不佳，请检查网络")}];
                error = err;
            }
            if (error.code == -1009) {
                // 网络断开
                NSError *err = [NSError errorWithDomain:@"LGF" code: -1009 userInfo:@{NSLocalizedDescriptionKey:SLLocalizedString(@"似乎已断开与互联网的连接")}];
                error = err;
            }
            if (error.code == 500) {
                NSError *err = [NSError errorWithDomain:@"LGF" code: 500 userInfo:@{NSLocalizedDescriptionKey:SLLocalizedString(@"当前服务不可用，请稍后再试")}];
                error = err;
            }
