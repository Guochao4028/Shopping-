...
//    window.scrollTo(0, newPos);
    ...
    /**
     2020-07-23  gc 重新计算 光标的位置，自测目前没发现问题，上面的注释是原第三方代码
     */
...
zss_editor.positionLast = function() {
...
zss_editor.e_insertImage = function(url, alt) {
    zss_editor.restorerange();
    var html = '<div><img src="'+url+'"/ onclick="zss_editor.e_imageClick()" onload="zss_editor.e_caculateHeight()"><div style="text-align: center;color: #888;"></div><hr style="border: 0;border-top: 0px solid #ccc;margin: 5px 0;"><div><br></div><div><br></div></div>';
    zss_editor.insertHTML(html);
    zss_editor.enabledEditingItems();
}

zss_editor.e_caculateHeight = function() {
    var height = document.body.offsetHeight;
    e_finishLoad(height);
}

zss_editor.e_setBlockquote = function() {
    var Range = document.getSelection().getRangeAt(0);
    formatName = Range.commonAncestorContainer.parentElement.nodeName === 'BLOCKQUOTE' ? 'DIV' : 'BLOCKQUOTE';
