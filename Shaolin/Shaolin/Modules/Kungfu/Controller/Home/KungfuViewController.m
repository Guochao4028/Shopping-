...
- (void)buildData {
    NSArray *array = @[
        [FieldModel createFieldModel:@"0" name:SLLocalizedString(@"段品制") showImg:@"kungfu_page_icon0" checkedImg:@"kungfu_page_icon0_select"],
        [FieldModel createFieldModel:@"1" name:SLLocalizedString(@"以武会友") showImg:@"kungfu_page_icon1" checkedImg:@"kungfu_page_icon1_select"],
        [FieldModel createFieldModel:@"2" name:SLLocalizedString(@"习武") showImg:@"kungfu_page_icon2" checkedImg:@"kungfu_page_icon2_select"],
        [FieldModel createFieldModel:@"3" name:SLLocalizedString(@"机构列表") showImg:@"kungfu_page_icon3" checkedImg:@"kungfu_page_icon3_select"],
        [FieldModel createFieldModel:@"4" name:SLLocalizedString(@"段品制介绍") showImg:@"kungfu_page_icon4" checkedImg:@"kungfu_page_icon4_select"],
    ];
...
//    self.pageViewController.view.frame = CGRectMake(0, 0, kWidth, kHeight - 10 - TabbarHeight);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.enabledTitles = array;// [FieldModel mj_objectArrayWithKeyValuesArray:array];
        // 需要延迟执行的代码
...
- (void) showTypeTable {
    ...
    [self.typeButton setImage:[UIImage imageNamed:@"kungfu_up_black"] forState:UIControlStateNormal];
    //    [self.typeButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 20)];
    //    [self.typeButton setImageEdgeInsets:UIEdgeInsetsMake(0, 45, 0, 0)];
...
- (void) hideTypeTable {
    self.bgAlphaView.hidden = YES;
    [self.typeButton setImage:[UIImage imageNamed:@"kungfu_down_black"] forState:UIControlStateNormal];
...
- (UIButton *)typeButton{
    if (!_typeButton){
        ...
        [_typeButton setTitleColor:KTextGray_333 forState:UIControlStateNormal];
        ...
        [_typeButton handleControlEvent:UIControlEventTouchUpInside block:^(UIButton * _Nonnull button) {
            if (weakSelf.typeTableBackView.height == 140) {
                ...
                [button setImage:[UIImage imageNamed:@"kungfu_down_black"] forState:UIControlStateNormal];
            } else {
                [weakSelf showTypeTable];
                [button setImage:[[UIImage imageNamed:@"kungfu_down_black"] lgf_ImageByRotate180] forState:UIControlStateNormal];
            }
        }];
    }
    return _typeButton;
