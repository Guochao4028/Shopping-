...
@property (nonatomic, strong) AVPTrackInfo *selectedMediaInfo;
@end
...
//        _downloadLabel.text = [SLLocalizedString(@"—————— 视频下载 ——————") localString];
...
//        [_lookVideoButton setTitle:[SLLocalizedString(@"查看离线视频") localString] forState:UIControlStateNormal];
...
//        _titleLabel.text = [SLLocalizedString(@"视频标题") localString];
    }
    return _titleLabel;
}
...
//        _selectInfoLabel.text = [SLLocalizedString(@"选择清晰度") localString];
    }
...
//        [_okButton setTitle:[SLLocalizedString(@"开始下载") localString] forState:UIControlStateNormal];
//        [_okButton setTitle:[SLLocalizedString(@"开始下载") localString] forState:UIControlStateSelected];
...
//        self.allContainView.frame = CGRectZero;
...
- (void)configBaseUI{
   ...
   // 数组根据 文件大小排序
    self.mediaInfos = [self.mediaInfos sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        AVPTrackInfo *m1 = obj1;
        AVPTrackInfo *m2 = obj2;
        if (m1.vodFileSize > m2.vodFileSize) {
    ...
    for (AVPTrackInfo *info in self.mediaInfos) {
...
- (void)layoutSubviews{
    ...
    if (allWidth < allHeight) {
        if (self.allContainView.frame.size.height == allHeight || self.allContainView.frame.size.height == allWidth) {
...
//        [self.okButton setBackgroundColor:[AlivcUIConfig shared].kAVCThemeColor];
        ...
        if (self.allContainView.frame.size.height == kContaniHeight) {
            [self removeFromSuperview];
            return;
        }
        //横屏
...
//    if (self.downloadSource.coverURL) {
//        [self.coverImageView sd_setImageWithURL:[NSURL URLWithString:self.downloadSource.coverURL]];
...
//        });
//    }
//    self.titleLabel.text = self.downloadSource.title;;
//    CGFloat mSize = (CGFloat)media.vodFileSize / 1024.0 / 1024.0;
//    NSString *mString = [NSString stringWithFormat:@"%.1fM",mSize];
//    self.downloadSource.trackIndex = media.trackIndex;
...
- (UIButton *)creatButtonWithMediaInfo:(AVPTrackInfo *)info{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button addTarget:self action:@selector(qualityButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
    NSString *title = [AVCSelectSharpnessView stringWithQuality:info.trackDefinition];
...
//        return [SLLocalizedString(@"流畅") localString];
//    }else if ([quality isEqualToString:@"LD"]) {
//        return [SLLocalizedString(@"标清") localString];
//    }else if ([quality isEqualToString:@"SD"]) {
//        return [SLLocalizedString(@"高清") localString];
//    }else if ([quality isEqualToString:@"HD"]) {
//        return [SLLocalizedString(@"超清") localString];
//    }else if ([quality isEqualToString:@"2K"]) {
//        return [@"2K" localString];
//    }else if ([quality isEqualToString:@"4K"]) {
//        return [@"4K" localString];
//    }else if ([quality isEqualToString:@"OD"]) {
//        return [SLLocalizedString(@"原画") localString];
//    }else if ([quality isEqualToString:@"SQ"]) {
//        return [SLLocalizedString(@"低音质") localString];
//    }else if ([quality isEqualToString:@"HQ"]) {
//        return [SLLocalizedString(@"高音质") localString];
//    }
    return SLLocalizedString(@"不明");
}
