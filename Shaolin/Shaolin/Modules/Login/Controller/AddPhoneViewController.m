...
- (void)finishButtonClick:(UIButton *)button{
    ...
    if (self.type == AddPhoneViewType_SetPassword){
        ...
        if (!self.codeTextField.text.length){
            ...
            return;
        } else if (self.codeTextField.text.length < 4){
            [ShaolinProgressHUD singleTextAutoHideHud:SLLocalizedString(@"请输入正确的验证码")];
        }
...
- (UITextField *)createTextField{
    ...
    textField.textColor = KTextGray_333;
...
- (UIButton *)sendCodeButton{
    if (!_sendCodeButton){
        ...
        [_sendCodeButton setTitle:SLLocalizedString(@"获取验证码") forState:UIControlStateNormal];
...
- (UIButton *)lookPasswordButton {
    if (!_lookPasswordButton) {
        ...
        [_lookPasswordButton setImage:[UIImage imageNamed:@"new_password_close"] forState:(UIControlStateNormal)];
        [_lookPasswordButton setImage:[UIImage imageNamed:@"new_password_open"] forState:(UIControlStateSelected)];
...
//        [_finishButton setBackgroundColor:kMainYellow];
        ...
        [_finishButton setBackgroundImage:[UIImage lgf_ColorImageWithFillColor:kMainLightYellow] forState:UIControlStateDisabled];
...
- (UILabel *)operationTypeLabel{
    if (!_operationTypeLabel){
        ...
        _operationTypeLabel.textColor = KTextGray_333;
