...
- (instancetype)init{
    ...
    if (self) {
        ...
        blackView1.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
        ...
        self.backView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, 260);
...
- (void)creadtionSub{
    ...
    [_CancerEnsure setTitleText:SLLocalizedString(@"出生年月日")];
    ...
    for (NSInteger i=0; i<12*100; i++) {
        NSString *monthStr = [NSString stringWithFormat:SLLocalizedString(@"%.2zd月"),(i)%12+1];
        [self.monthArr addObject:monthStr];
    }
    
    for (NSInteger i=0; i<31*100; i++) {
        NSString *dayStr = [NSString stringWithFormat:SLLocalizedString(@"%.2zd日"),(i)%31+1];
        [self.dayArr addObject:dayStr];
    }
    
    for (NSInteger i=1800; i<2300; i++) {
        NSString *yearStr = [NSString stringWithFormat:SLLocalizedString(@"%zd年"),i];
...
- (void)startAnimationFunction{
    ...
    rect.origin.y = ScreenHeight-260;
    [self.window bringSubviewToFront:self];
