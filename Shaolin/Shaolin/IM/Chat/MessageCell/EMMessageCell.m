...
- (void)_setupViewsWithType:(EMMessageType)aType
{
    ...
    _avatarView.cornerRadius = 4.0f;
    _avatarView.clipsToBounds = YES;
    [self.contentView addSubview:_avatarView];
    if (self.direction == EMMessageDirectionSend) {
//        _avatarView.image = [UIImage imageNamed:@"user_avatar_me"];
        [_avatarView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[SLAppInfoModel sharedInstance].headurl]] placeholderImage:[UIImage imageNamed:@"shaolinlogo"]];
        ...
        _avatarView.image = [UIImage imageNamed:@"navigationBar_leftIcon"];
        [_avatarView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.contentView).offset(15);
...
- (void)setModel:(EMMessageModel *)model
{
    ...
    if (model.direction == EMMessageDirectionSend) {
        ...
        NSLog(@"model.emModel.ext = %@", model.emModel.ext);
        
        
        if ([model.emModel.ext isKindOfClass:[NSDictionary class]] == YES) {
