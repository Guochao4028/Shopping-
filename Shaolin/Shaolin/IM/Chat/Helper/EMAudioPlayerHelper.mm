...
{
    if (self.playerFinished) {
        NSError *error = [NSError errorWithDomain:SLLocalizedString(@"播放失败!") code:-1 userInfo:nil];
                 ...
                 completion:(void(^)(NSError *error))aCompleton
{
    ...
    do {
        ...
        if (![fm fileExistsAtPath:aPath]) {
            error = [NSError errorWithDomain:SLLocalizedString(@"文件路径不存在") code:-1 userInfo:nil];
            break;
        }
        ...
        if ([aPath length] == 0) {
            error = [NSError errorWithDomain:SLLocalizedString(@"转换音频格式失败") code:-1 userInfo:nil];
            break;
        }
        ...
        if (error || !self.player) {
            self.player = nil;
            error = [NSError errorWithDomain:SLLocalizedString(@"初始化AVAudioPlayer失败") code:-1 userInfo:nil];
            break;
        }
        ...
        if (!ret) {
            [self stopPlayer];
            error = [NSError errorWithDomain:SLLocalizedString(@"AVAudioPlayer播放失败") code:-1 userInfo:nil];
        }
